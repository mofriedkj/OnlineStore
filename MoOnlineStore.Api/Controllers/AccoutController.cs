...
namespace MoOnlineStore.Api.Controllers
{
    ...
    public class AccountController : BaseApiController
    {
        ...
        public async Task<ActionResult<UserDto>> Login(LoginDto logindto)
        {
            ...
            if (user == null){
                return Unauthorized(new ApiResponse(401, "Password or email not correct"));
            }
            var result = await _signInManager.CheckPasswordSignInAsync(user, logindto.Password, true);  
            if(result.IsLockedOut) return Unauthorized(new ApiResponse(401,"lockedout"));
            if (!result.Succeeded) return Unauthorized(new ApiResponse(401,"Password or email not correct"));
        ...
        public async Task<ActionResult<UserDto>> resister(RegisterDto registerdto)
        {
            ...
            if (!result.Succeeded) return BadRequest(new ApiResponse(401,"Problem By Register"));
            return new UserDto { 
                DisplayName = user.DisplayName, 
                Email = user.Email, 
                Token = _tokenService.GenarateToken(user)};
